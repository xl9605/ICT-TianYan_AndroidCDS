...
@app.route('/predict/<string:label>', methods=['POST'])
def predict(label):
    ...
    filepath=UPLOAD_FOLDER+label+'/lpms.csv'
    final_table.to_csv(filepath, header=False, index=False, mode='a')
...
def robot(name):
   ...
   # print (ress)
    if(res == 0):
        if (comparetwoimage.compare(ress, name)):
            robotdata = {"cmd": res, "device_code": Transformer_Table.getTransformID(ress), "time": resss}
            headers = {'Content-Type': 'application/json'}
            robot_request = requests.post(url=urllib.request.quote('http://127.0.0.1:56789/robotReceive', ':/=&?'),
                                          headers=headers,
                                          data=urllib.parse.urlencode(robotdata).encode(encoding='UTF8'))
            get_robot_json = robot_request.json()
            result_get_robot_json = get_robot_json.get('result')
            if (result_get_robot_json == "success"):
                print("成功!")
                data = {"result": "success", "value": "操作成功！"}
        else:
            data = {"result": "fail", "value": "操作失败！请重新操作！"}
    else:
        data = {"result": "success", "value": "操作成功！"}
...
@app.route('/ocr/<string:name>', methods=['POST'])
def ocr_result(name):
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            m_path = os.path.join(filepath, filename)
            file.save(m_path)
                    ...
                    if flag==1:
                        ...
                        while i<=15:
                            ...
                            if rc12:
                                ...
                                print("asdasdasdasdasdfa"+str(insertop))
                        ...
                        if (int(insertop)):
                            sqlcommand = 'delete from operation where worktableID=%s;' % ('"' + str(main_tableId) + '"')
                            database_connect()
                            deletewt = cur.execute(sqlcommand)
                            database_disconnect()
                            if (int(deletewt)):
                                database_connect()
                                sqlcommand = 'delete from worktable where worktableID=%s;' % (
                                            '"' + str(main_tableId) + '"')
                                deletewt = cur.execute(sqlcommand)
                                database_disconnect()
...
def face_upload():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            filepath=UPLOAD_FOLDER+timestr+'/'
            os.mkdir(filepath)
            m_path = os.path.join(filepath, filename)
            file.save(m_path)
            return timestr
...
if __name__ == '__main__':
    app.run(port=23456, host='10.41.0.128')
