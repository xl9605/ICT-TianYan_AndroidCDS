package ac.ict.humanmotion.abracadabra.OCR
...
class OCRActivity : BaseActivity() {
    ...
    private fun OCRthread() {
        ...
        thread {
            ...
            Imgproc.cvtColor(srcMat, temp, Imgproc.COLOR_RGBA2GRAY)
            Imgproc.Sobel(temp, temp, CvType.CV_8U, 1, 0)
            Imgproc.Sobel(temp, temp, CvType.CV_8U, 0, 1)
            Imgproc.threshold(temp, temp, 0.0, 255.0, Imgproc.THRESH_OTSU + Imgproc.THRESH_BINARY)

            val element1 = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, Size(30.0, 2.0))
            val element2 = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, Size(24.0, 3.0))

            Imgproc.dilate(temp, temp, element2)

            Imgproc.erode(temp, temp, element1)

            for (i in 0..8) Imgproc.dilate(temp, temp, element2)

            val rects: MutableList<RotatedRect> = mutableListOf()
            val contours: MutableList<MatOfPoint> = mutableListOf()

            Imgproc.findContours(temp, contours, Mat(), Imgproc.RETR_CCOMP, Imgproc.CHAIN_APPROX_SIMPLE)
            ...
            contours.forEach {
                ...
                Imgproc.approxPolyDP(newPoint, MatOfPoint2f(), 0.001 * Imgproc.arcLength(newPoint, true), true)

                val rect = Imgproc.minAreaRect(newPoint)
...
//                if (m_height > m_width * 1.2) return@forEach
            ...
            rects.forEach {
                ...
                for (j in 0..3) Imgproc.line(srcMat, pt[j], pt[(j + 1) % 4], Scalar(0.0, 255.0, 0.0), 2)
            }
