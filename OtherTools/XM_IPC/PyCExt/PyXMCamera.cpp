...
std::map<string,string> search_device_table;//时时监控到的设备MAC-IP键值对对应表

//设定的MAC-IP键值对对应表
std::map<string,string> device_table;
std::map<string,string> device_IPChanged;

//device_table.insert(pair<string, string>("00:12:17:b4:18:6d","10.41.0.166"));
//device_table.insert(pair<string, string>("00:12:17:c0:2c:cc","10.41.0.170"));
//device_table.insert(pair<string, string>("00:12:17:c8:c2:ab","10.41.0.167"));
//device_table.insert(pair<string, string>("00:12:17:c8:c7:c4","10.41.0.168"));
//device_table.insert(pair<string, string>("00:12:17:c8:c7:c4","10.41.0.168"));
...
bool isIPChanged()
{
	std::map<string,string>::iterator it = search_device_table.begin();
	while( it != search_device_table.end())
	{
		std::map<string,string>::iterator itDst = device_table.find(it->first);
		if (itDst != device_table.end())
			if (itDst->second != it->second)
			{
			    device_IPChanged.insert(pair<string, string>(it->second,itDst->second));
			    return true;
			}
		it++;
	}
...
static PyObject *CXMCamera_SetIP(CXMCamera *Self)
{
}
static PyObject *CXMCamera_SearchDevice(CXMCamera *Self)
{
    SDK_CONFIG_NET_COMMON_V2 m_Device[100];
	int nRetLength = 0;

	int changeIP=H264_DVR_SetLocalBindAddress("10.41.0.166");
	printf("IP地址改变的标志:%d\n",changeIP);
	cout<<"-------------Start Search IPC Device In Lan.-------------"<<endl;
	std::string IPV4_Addr="";
	bool bRet= H264_DVR_SearchDevice((char*)m_Device,sizeof(SDK_CONFIG_NET_COMMON_V2)*100,&nRetLength,5000);
	if(bRet)
	{
	    for(int i = 0;i < nRetLength/sizeof(SDK_CONFIG_NET_COMMON_V2);i++)
	    {
	        printf("-----------------------------------\n");
	        IPV4_Addr=std::to_string((int)m_Device[i].HostIP.c[0])+"."+std::to_string((int)m_Device[i].HostIP.c[1])+"."+std::to_string((int)m_Device[i].HostIP.c[2])+"."+std::to_string((int)m_Device[i].HostIP.c[3]);
	        cout<<"m_Device"<<i<<"->HostIP:"<< IPV4_Addr <<endl;
	        string mac=m_Device[i].sMac;
		    cout<<"m_Device"<<i<<"->HostMAC："<<m_Device[i].sMac<<endl;
		    search_device_table.insert(pair<string, string>(mac,IPV4_Addr));
	    }
		printf("-----------------------------------\nH264_DVR_SearchDevice ok number is [%d]\n",nRetLength/sizeof(SDK_CONFIG_NET_COMMON_V2));
	    for(map<string, string>::iterator it = search_device_table.begin(); it != search_device_table.end(); it++)
        {
            cout<<it->first.c_str()<<", "<<it->second.c_str()<<endl;
        }
        if(isIPChanged())
        {
            printf("IP Is Changed!!!\n");
            for(map<string, string>::iterator it = device_IPChanged.begin(); it != device_IPChanged.end(); it++)
                cout<<it->first.c_str()<<", "<<it->second.c_str()<<endl;

        }
        else
            printf("All IP Is Not Changed!!!\n");
	}
	cout<<"-------------Search IPC Device In Lan Ended.-------------"<<endl;
	Py_XINCREF(Py_None);
	return Py_None;
}
...
static PyMethodDef CXMCamera_MethodMembers[] =
	{
		...
		{"SearchDevice", (PyCFunction)CXMCamera_SearchDevice, METH_NOARGS, "Search H264Device in LAN."},
        {NULL, NULL, NULL, NULL}};
