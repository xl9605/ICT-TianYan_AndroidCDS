...
public class RecyclerAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public RecyclerAdapter(Context context, List<WorkListDetail> dataBeanList) {
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        switch (getItemViewType(position)){
            ...
            case WorkListDetail.CHILD_ITEM:
                ...
                childViewHolder.bindView(dataBeanList.get(position), position,itemClickListener);
                break;
        }
    }
    ...
    private ItemClickListener itemClickListener = new ItemClickListener() {
        @Override
        public void onExpandChildren(WorkListDetail bean) {
            int position = getCurrentPosition(bean.getID());//确定当前点击的item位置
            WorkListDetail children = getChildDataBean(bean);//获取要展示的子布局数据对象，注意区分onHideChildren方法中的getChildBean()。
            if (children == null) {
                return;
            }
        ...
        @Override
        public void onHideChildren(WorkListDetail bean) {
            int position = getCurrentPosition(bean.getID());//确定当前点击的item位置
            WorkListDetail children = bean.getChildBean();//获取子布局对象
            if (children == null) {
                return;
            }
     ...
     * @param position
     */
    public void add(WorkListDetail bean, int position) {
     ...
     * 要和onHideChildren方法e里的getChildBean()区分开来
     * @param bean
     * @return
     */
    private WorkListDetail getChildDataBean(WorkListDetail bean){
        WorkListDetail child = new WorkListDetail();
        child.setType(1);
        child.setWorkListName(bean.getWorkListName());
        child.setSupervisor(bean.getSupervisor());
        child.setOprationPerson(bean.getOprationPerson());
        child.setStartTime(bean.getStartTime());
        child.setTime(bean.getTime());
        child.setFinishTime(bean.getFinishTime());
        return child;
    }
